...
export default function MarketWatch({ niftySpot, bankNiftySpot, indiaVix, futuresOi, indicators }) {
    ...
    <div className="flex flex-col space-y-4 h-full font-mono">
      ...
      <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {/* NIFTY 50 */}
          <div className={`p-3 rounded-xl border border-slate-800/80 transition-all duration-300 ${niftyFlash || 'bg-slate-950/40'}`}>
            <span className="text-[9px] text-slate-500 tracking-wider">NIFTY 50</span>
            <div className="flex items-baseline justify-between mt-1">
              <span className="text-md font-bold text-slate-100">
                {niftySpot ? niftySpot.toFixed(1) : 'Loading...'}
              </span>
            </div>
          </div>

          {/* BANK NIFTY */}
          <div className={`p-3 rounded-xl border border-slate-800/80 transition-all duration-300 ${bankFlash || 'bg-slate-950/40'}`}>
            <span className="text-[9px] text-slate-500 tracking-wider">BANK NIFTY</span>
            <div className="flex items-baseline justify-between mt-1">
              <span className="text-md font-bold text-slate-100">
                {bankNiftySpot ? bankNiftySpot.toFixed(1) : 'Loading...'}
              </span>
            </div>
          </div>
        ...
        {indicators && vwap ? (
          <div className="space-y-3">
            ...
            <div className="bg-slate-950/30 p-2.5 rounded-xl border border-slate-900 space-y-1.5">
              <span className="text-xs font-semibold text-slate-300 block mb-0.5">Key Levels (30m Range)</span>
